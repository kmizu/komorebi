...
import { z } from 'zod';
import { saveSession, updatePostOutcome } from '@/lib/db/queries';
...
export async function PATCH(req: NextRequest) {
  try {
    const body = await req.json();
    const { id, postOutcome } = UpdateSessionSchema.parse(body);

    // Generate summary if LLM available
    let summary: string | undefined;
    if (isLLMConfigured()) {
      try {
        const session = await import('@/lib/db/queries').then(m => m.getSessionById(id));
        ...
        summary = await complete('Generate a brief factual session summary.', prompt, 60);
